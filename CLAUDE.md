...
- 커밋은 목적이 하나여야 하며(`fix`, `docs`, `refactor` 등), 성격이 다른 변경은 같은 브랜치/PR에 섞지 않는다.
- 긴급 수정(`hotfix/*`) 브랜치에는 배포·장애 대응에 필요한 수정만 포함하고, 문서/가이드 변경은 별도 `docs/*` 브랜치로 분리한다.
- 브랜치의 목적과 커밋 타입이 불일치하면 머지 전에 브랜치를 분리하거나 커밋을 재구성한다.
